var logger = require("log4js").getLogger("pro-xy-url-replace");
	...
	if (urlReplConfig.replaceBackHeaders && urlReplConfig.replaceBackHeaders.length) {
		...
		res.writeHead = function() {
			urlReplConfig.replaceBackHeaders.forEach(headerName => {
				...
				headerValues = headerValues.map(headerName => {
					var newVal = headerVal.replace(diff.b, diff.a);
					logger.warn(`Replacing back header ${headerVal} val '${headerVal}' for '${newVal}'`);
					return newVal;
				});
